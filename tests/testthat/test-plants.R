test_that("optimize_model does not throw error on
          gaussian backward simplification with psi", {
            ...
            autocor_cols <- c("solar_radiation",
            ...
            expect_warning(
              res <- optimize_model(df = plants,
                                    term = term,
                                    autocorrelation_cols = autocor_cols,
            ...
            expect_equal(names(res$models_with_info),
                         c("backward"))
            expect_equal(
                         names(res$models_with_info$backward),
                         c("overview",
                           "final_model",
                           "plots",
                           "history",
...
test_that("optimize_model does not throw error on gaussian
    ...
    res <- optimize_model(df = plants,
                          term = term,
                          autocorrelation_cols = autocorrelation_cols,
                          automatic_removal = TRUE,
                          autocorrelation_threshold = 0.8,
                          correlation_method = "spearman",
                          model_type = "glm",
...
test_that("optimize_model does not throw error on binomial
